...
impl Screen {
    ...
    /// Returns a reference to the visible row at index `row` (respects
    /// scrollback offset). Used for efficient per-cell rendering without
    /// cloning the entire screen.
    #[must_use]
    pub fn visible_row(&self, row: u16) -> Option<&crate::row::Row> {
        self.grid().visible_row(row)
    }

    /// Returns an iterator over all visible rows (respects scrollback offset)
    /// in a single O(scrollback_skip + rows_len) pass.  Prefer this over
    /// repeated `visible_row(r)` calls, which each recreate the iterator and
    /// cost O(scrollback_skip + r) per call — O(rows²) total.
    pub fn visible_rows_iter(&self) -> impl Iterator<Item = &crate::row::Row> {
