...
impl Grid {
    ...
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
        ...
        // Cap scrollback rows at rows_len so screen_rows never underflows.
        // When scrollback_offset > rows_len the entire visible area comes from
        // the scrollback buffer, shifted further back in history.
        let scrollback_rows = self.scrollback_offset.min(rows_len);
        let screen_rows = rows_len - scrollback_rows;
        let scrollback_skip = scrollback_len.saturating_sub(self.scrollback_offset);
        self.scrollback
            .iter()
            .skip(scrollback_skip)
            .take(scrollback_rows)
            .chain(self.rows.iter().take(screen_rows))
    }
